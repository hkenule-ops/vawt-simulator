...
def test_analyze_economics_endpoint(client):
    r = client.post("/api/v1/economics/analyze", json={
        "geometry": SAMPLE_GEOMETRY, "material": "CFRP_UD", "ply_material": "CFRP_UD_PLY",
        "electricity_price_usd_per_kwh": 0.15, "discount_rate": 0.06, "project_lifetime_years": 20,
    })
    assert r.status_code == 200
    body = r.json()
    assert body["aep"]["aep_kwh"] > 0
    assert body["capex"]["total_capex_usd"] > 0
    assert body["lcoe_usd_per_kwh"] > 0


def test_analyze_economics_unknown_material_returns_400(client):
    r = client.post("/api/v1/economics/analyze", json={
