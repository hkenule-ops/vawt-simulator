...
def test_analyze_aeroelastics_endpoint(client):
    r = client.post("/api/v1/aeroelastic/analyze-blade", json={
        "geometry": SAMPLE_GEOMETRY, "material": "CFRP_UD", "operating_tsr": 2.25,
    })
    assert r.status_code == 200
    body = r.json()
    assert len(body["modal"]["natural_frequencies_hz"]) == 4
    assert body["operating_rpm_max"] > body["operating_rpm_min"]
    assert 1 in body["harmonics"]["dominant_harmonics"]


def test_analyze_aeroelastics_unknown_material_returns_400(client):
    r = client.post("/api/v1/aeroelastic/analyze-blade", json={
        "geometry": SAMPLE_GEOMETRY, "material": "UNOBTANIUM", "operating_tsr": 2.25,
    })
