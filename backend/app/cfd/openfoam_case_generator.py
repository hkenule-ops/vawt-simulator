...
def generate_readme(geom: HybridRotorGeometry, cfg: CFDCaseConfig) -> str:
    d = geom.darrieus
    if d.twist_angle_deg == 0.0 and d.helical_twist_deg == 0.0:
        blade_shape_note = "straight blade (no twist/helical sweep)"
    else:
        blade_shape_note = (
            f"twisted/helical blade -- {d.twist_angle_deg:.1f} deg linear twist "
            f"root-to-tip, {d.helical_twist_deg:.1f} deg helical sweep root-to-tip"
        )
...
## Files included
- `blade.stl` - lofted blade geometry (see "Blade geometry" above)
